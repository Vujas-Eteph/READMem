...
class EvalMemoryReader(nn.Module):
    ...
    def forward(self, mk, mv, qk):
        ...
        self.affinity = affinity.clone()  # B, THW, HW
...
class PropagationNetwork(nn.Module):
    ...
    def memorize(self, frame, masks): 
        ...
        f16_score = F.interpolate(f16, [24, 24], mode='bilinear')   #feature for quality assessment # F.interpolate() --> Down/up samples the input to either the given size
        # torch.save(f16_score, 'f16_score.pt')
        mask_score = self.score(f16_score)  # Compute the score
    ...
    def get_affinity(self):
        return self.memory.get_affinity()
